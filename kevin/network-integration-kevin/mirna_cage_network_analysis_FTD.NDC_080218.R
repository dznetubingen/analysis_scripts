#################################################
## miRNA-mRNA network analysis using CAGE data ##
#################################################
# first created on 08-02-2018
...
for (m in rownames(mirna.sig)){
  ...
  mir.targets <- mir.targets[mir.targets %in% rownames(cage.sig)]
  mir.target.counts <- na.omit(cage[rownames(cage) %in% mir.targets,])
...
for (i in 1:nrow(tf.gene.flt)) {
  ...
  if (tf %in% rownames(cage) && gene %in% rownames(cage)){
    tf.cts <- cage[rownames(cage) == tf,]
    gene.cts <- cage[rownames(cage) == gene,]
    ...
    if (tf.cor$p.value <= pval_cutoff){
      source.lfc = 0
      if (tf %in% rownames(cage.deg)){
        source.lfc <- cage.deg[rownames(cage.deg) == tf,]$log2FoldChange
      } 
...
for (i in 1:length(source.prots)){
  prot <- source.prots[i]
  lfc = 0
  if (prot %in% rownames(cage.deg)){
    lfc <- cage.deg[rownames(cage.deg) == prot,]$log2FoldChange
  }
  lfcs <- c(lfcs, lfc)
}
...
#########################################
### Add TF-miRNA mappings (RegNetwork) ##
#########################################
...
for (i in 1:length(tfs)){
  ...
  lfc = 0
  if (tf %in% rownames(cage.deg)){
    lfc <- cage.deg[rownames(cage.deg) == tf,]$log2FoldChange
  }
  lfcs <- c(lfcs, lfc)
}
...
# Save files
write.table(network, "~/rimod/smallRNA/analysis/network_analysis/cage_network_analysis/mirna_cage_network_cor0.5_pval0.05_network.txt", sep="\t", row.names=F, quote=F)
...
# Test GML format
write_graph(g, "~/rimod/smallRNA/analysis/network_analysis/cage_network_analysis/mirna_cage_network_cor0.5_pval0.05_080218.gml", format = "gml")
