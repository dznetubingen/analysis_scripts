#################################
# Membrane trafficking investigation
#################################
library(pheatmap)
library(stringr)
library(viridis)
...
# load pathway
mt <- read.table("membrane_trafficking_reactome.tsv", sep="\t", header=T)
genes <- str_split(mt$MoleculeName, pattern=" ", simplify = T)[,2]
mt$name <- genes
# Get ensembl IDs
bm <- getBM(attributes = c("ensembl_gene_id", "hgnc_symbol"), filters="hgnc_symbol", values=genes, mart=ensembl)
mt <- merge(mt, bm, by.x="name", by.y="hgnc_symbol")
mt <- mt[!duplicated(mt$ensembl_gene_id),]
rownames(mt) <- mt$ensembl_gene_id
...
write.table(rownames(mat.grn), "GRN_MembTraf_DEGs.txt", row.names = F, col.names = F, quote=F)
...
as.grn <- as.grn[as.grn$x %in% mt$ensembl_gene_id,]

as.mapt <- read.table("~/rimod/RNAseq/as_analysis/majiq/mapt_AS_genes_dPSI_0.2.txt", sep="\t", header = T)
as.mapt <- as.character(as.mapt[as.mapt$x %in% mt$ensembl_gene_id,])
...
ps.grn <- as.character(ps.grn[ps.grn$x %in% mt$ensembl_gene_id,])
ps.grn <- mt[ps.grn,]
# mapt
ps.mapt <- read.table("~/rimod/CAGE/cage_analysis/promotor_shifting/frontal/mapt_promotor_shifting_genes_fro.txt", header=T)
ps.mapt <- as.character(ps.mapt[ps.mapt$x %in% mt$ensembl_gene_id,])
ps.mapt <- mt[ps.mapt,]
...
mapt.met.genes <- mapt.met.genes[mapt.met.genes %in% mt$name]
...
grn.met.genes <- grn.met.genes[grn.met.genes %in% mt$name]
...
mapt.anno <- mapt.anno[mapt.anno$SYMBOL %in% mt$name,]
...
grn.anno <- grn.anno[grn.anno$SYMBOL %in% mt$name,]
...
c9.anno <- c9.anno[c9.anno$SYMBOL %in% mt$name,]
...
tf.grn.wnt <- tf.grn[tf.grn$TF %in% mt$name,]
...
exp.grn <- exp.grn[exp.grn$X %in% mt$ensembl_gene_id,]
exp.grn <- merge(exp.grn, mt, by.x="X", by.y="ensembl_gene_id")
...
# get PPI connections
ppi <- read.table("membrane_trafficking_STRING.tsv", sep="\t", header=T)
ppi <- ppi[, c(1,2,15)]
...
# Save file
file_name = "GRN_MembraneTraficking_signaling_network.gml"
