##################
# Protein Ubiquitination (Reactome) analysis
#################
...
setwd("~/rimod/integrative_analysis/protein_ubiquitination/")
...
ubi <- read.table("protein_ubiquitination_reactome.tsv", sep="\t", header=T)
...
exp.grn <- exp.grn[exp.grn$X %in% ubi$converted_alias,]
...
# get PPI connections
ppi <- read.table("protein_ubiquitination_string.txt", sep="\t", header=T)
ppi <- ppi[, c(1,2,15)]
...
# Save file
file_name = "GRN_ProteinUbiquitination_network.gml"
