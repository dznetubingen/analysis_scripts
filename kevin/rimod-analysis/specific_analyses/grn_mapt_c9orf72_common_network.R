##################
# Analysis of STRING network conncting MAPT, C9orf72 and GRN
#################
library(pheatmap)
library(stringr)
library(viridis)
...
setwd("~/rimod/integrative_analysis/grn_mapt_c9orf72_network/")
...
ubi <- read.table("string_interactions.tsv", sep="\t")
genes <- as.character(ubi$V1)
ubi$name <- genes
# Get ensembl IDs
