...
setwd("~/rimod/CAGE/cage_analysis/promotor_shifting/temporal//")
...
# Load results
mapt <- read.table("mapt_shifting_promotors_tem.txt", sep="\t", header=T)
grn <- read.table("grn_shifting_promotors_tem.txt", sep="\t", header=T)
c9 <- read.table("c9_shifting_promotors_tem.txt", sep="\t", header=T)
...
# Save results
write.table(mapt.df$ENSEMBL, "mapt_promotor_shifting_genes_tem.txt", quote=F, row.names=F)
write.table(grn.df$ENSEMBL, "grn_promotor_shifting_genes_tem.txt", quote=F, row.names=F)
write.table(c9.df$ENSEMBL, "c9_promotor_shifting_genes_tem.txt", quote=F, row.names=F)
