...
write.table(as.data.frame(ccl.df), "mouse_grn_CAGE_clusters.txt", sep="\t", quote=F, col.names = NA)
...
write.table(peakFile, "mouse_grn_CAGE_cluster_peaks.bed", sep="\t", quote=F, row.names = F, col.names = F)

# generate peak object
peaks <- readPeakFile("mouse_grn_CAGE_cluster_peaks.bed")
