...
# with added deconvolution data to account for cell composition
# biases
##########################################################
library(tximport)
library(DESeq2)
...
region <- "fro"
fractions <- "/home/kevin/rimod/RNAseq/analysis/deconvolution/cdn_predictions.txt"
...
dir.create(paste("Deconvolved_RNAseq_analysis","_",region, "_", current_time, sep=""))
setwd(paste("Deconvolved_RNAseq_analysis", "_", region, "_",current_time, sep=""))

# Save parameters in config file
params <- c(current_time, as.character(row_sum_cutoff), metadata, salmon_files, analysis_dir, script_name, region, fractions)
param.names <- c("Time", "Row_sum_cutoff", "Metadata", "salmon_files", "Analysis_directory", "Script_name", "Region", "Fractions")
...
# Load cell compositions
fracs <- read.table(fractions)
fracs <- fracs[colnames(cts),]
fracs$Neurons <- fracs$InNeurons + fracs$ExNeurons
fracs <- fracs[, c(-1,-2,-7,-8)]

# bin fractions for comparison
fracs.binned <- fracs
fracs.binned$Neurons <- make.names(cut(fracs.binned$Neurons, breaks=5))
fracs.binned$Oligodendrocytes <- make.names(cut(fracs$Oligodendrocytes, breaks=5))
fracs.binned$Endothelial <- make.names(cut(fracs.binned$Endothelial, breaks=5))
fracs.binned$Microglia <- make.names(cut(fracs.binned$Microglia, breaks=5))
fracs.binned$Astrocytes <- make.names(cut(fracs.binned$Astrocytes, breaks=5))
...
dds <- DESeqDataSetFromMatrix(cts,
                              colData = md,
                              design = ~ Neurons + AGE.BIN + GENDER + DISEASE.CODE)
...
deg.mapt <- res.mapt[res.mapt$pvalue <= 0.001,]
...
deg.grn <- res.grn[res.grn$pvalue <= 0.001,]
...
deg.c9 <- res.c9[res.c9$pvalue <= 0.001,]
...
###
# Generate files for use with STRING-DB
###
# MAPT
deg.mapt <- res.mapt[res.mapt$pvalue <= 0.05,]
deg.mapt <- deg.mapt[order(deg.mapt$log2FoldChange, decreasing = T),]
deg.mapt.df <- data.frame(gene = rownames(deg.mapt), lfc = deg.mapt$log2FoldChange)
write.table(deg.mapt.df, "STRING_MAPT_genes.txt", sep="\t", quote=F, row.names = F)

# GRN
deg.grn <- res.grn[res.grn$pvalue <= 0.05,]
deg.grn <- deg.grn[order(deg.grn$log2FoldChange, decreasing = T),]
deg.grn.df <- data.frame(gene = rownames(deg.grn), lfc = deg.grn$log2FoldChange)
write.table(deg.grn.df, "STRING_GRN_genes.txt", sep="\t", quote=F, row.names=F)

# C9orf72
deg.c9 <- res.c9[res.c9$pvalue <= 0.05,]
deg.c9 <- deg.c9[order(deg.c9$log2FoldChange, decreasing = T),]
deg.c9.df <- data.frame(gene = rownames(deg.c9), lfc = deg.c9$log2FoldChange)
write.table(deg.c9.df, "STRING_C9orf72_genes.txt", sep="\t", quote=F, row.names=F)
...
gmt_c2 <- gmtPathways("~/resources/genesets/c2.all.v6.1.entrez.gmt")
pval_filter <- 0.05
...
# mapt C2
mapt.gsea <- fgsea(gmt_c2, ranks, minSize=15, maxSize=500, nperm=1000)
mapt.gsea <- mapt.gsea[order(mapt.gsea$pval)]
mapt.gsea <- as.data.frame(mapt.gsea)
mapt.gsea <- mapt.gsea[, -ncol(mapt.gsea)] # get rid of last column
write.table(mapt.gsea, "fGSEA_results_C2_MAPT.txt", sep="\t", quote=F)
...
# grn C2
grn.gsea <- fgsea(gmt_c2, ranks, minSize=15, maxSize=500, nperm=1000)
grn.gsea <- grn.gsea[order(grn.gsea$pval)]
grn.gsea <- as.data.frame(grn.gsea)
grn.gsea <- grn.gsea[, -ncol(grn.gsea)] # get rid of last column
write.table(grn.gsea, "fGSEA_results_C2_GRN.txt", sep="\t", quote=F)
...
# C9orf72 C2
c9.gsea <- fgsea(gmt_c2, ranks, minSize=15, maxSize=500, nperm=1000)
c9.gsea <- c9.gsea[order(c9.gsea$pval),]
c9.gsea <- as.data.frame(c9.gsea)
c9.gsea <- c9.gsea[, -ncol(c9.gsea)] # get rid of last column
write.table(c9.gsea, "fGSEA_results_C2_c9orf72.txt", sep="\t", quote=F)
