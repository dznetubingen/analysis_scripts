...
setwd("~/tmp_stuff/nanopore_tmp/strique_results_220620/")
...
# DN19 C1
###
strique <- read.table("c9orf72_DN19C1.strique.tsv", sep="\t", header=T)
...
# DN19 C2
###
strique <- read.table("c9orf72_DN19C2.strique.tsv", sep="\t", header=T)
strique <- strique[order(strique$count, decreasing = T),]
# remove small expansions
strique <- strique[strique$count > cutoff,]
df <- data.frame(expansion = strique$count, strand = strique$strand)
...
# Rotterdam sample 70
###
strique <- read.table("c9orf72_rotterdam70.strique.tsv", sep="\t", header=T)
strique <- strique[order(strique$count, decreasing = T),]
# remove small expansions
strique <- strique[strique$count > cutoff,]
df <- data.frame(expansion = strique$count, strand = strique$strand)
p <- ggplot(df, aes(x=expansion, color=strand, fill=strand)) +
  geom_histogram(alpha=0.5, bins=50)
p
ggsave(filename = "Rotterdam70_histogram_STRique_hg38.png")
...
p <- ggplot(df, aes(x=expansion, color=strand, fill=strand)) +
  geom_histogram(alpha=0.5, bins=50)
p
ggsave(filename = "Rotterdam70_histogram_STRique_hg38_noCutoff.png")
